...
export default function Weather(props) {
  ...
  let dayForecast =
    forecast.forecast.txt_forecast.forecastday[0].fcttext_metric;
  let nightForecast =
    forecast.forecast.txt_forecast.forecastday[1].fcttext_metric;

  return (
    <div className="weather">
      <h1 className="weather__city"> {city}</h1>
      <img className="weather__icon" src={icon} />
      <h2 className="weather__todayHigh">{fahrenheit}°F</h2>
      <h2 className="weather__todayLow">{lowTemp}°F</h2>
      <h2 className="weather__fullForecast">
        {dayForecast} and tonight {nightForecast}{' '}
      </h2>
